...
;                          RODRIGO BOMBARDI
;
; Subroutine that generates the modeled variogram using the spherical
...
;====================================================================
pro model_sphere,xvar,par,yvar,dpar
...
yvar(*)=par(1)+par(2)
if(cas2 gt 0)then yvar(cas)=par(1)+par(2)*(1.5*xvar(cas)/par(0)-0.5*(xvar(cas)/par(0))^3)

res=size(xvar)
if(res(0) eq 1)then begin
dpar=fltarr(res(1),3)
if(cas2 gt 0)then dpar(cas,0)=1.5*par(2)*xvar(cas)*((xvar(cas)^2)/(par(0)^2)-1)/(par(0)^2)
dpar(*,1)=1.0
if(cas2 gt 0)then dpar(cas,2)=xvar(cas)*(3-(xvar(cas)^2)/(par(0)^2))/(2*par(0))
endif
end
;====================================================================
